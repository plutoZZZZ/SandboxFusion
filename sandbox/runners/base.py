...
async def run_commands(compile_command: Optional[str], run_command: str, cwd: str, extra_env: Optional[Dict[str, str]],
    ...
    if config.sandbox.isolation == 'none':
        ...
        _, hard_memory_limit_DATA = resource.getrlimit(resource.RLIMIT_AS)
        ...
        if platform.uname().system != "Darwin":
            _, hard_memory_limit_AS = resource.getrlimit(resource.RLIMIT_STACK)
            resource.setrlimit(resource.RLIMIT_STACK, (soft_memory_limit, hard_memory_limit_AS))
